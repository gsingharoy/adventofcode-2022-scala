...
class GameActionSpec extends  AnyFlatSpec with Matchers {
  ...
  "GameAction.constructFromString" should "result in a success for cases where two actions are formed" in {
    ...
    GameAction.constructFromString("A X", config1, config2) shouldEqual Some(GameAction(Rock(), Rock()))

    GameAction.constructFromString("A Y", config1, config2) shouldEqual Some(GameAction(Rock(), Paper()))

    GameAction.constructFromString("A Z", config1, config2) shouldEqual Some(GameAction(Rock(), Scissors()))

    GameAction.constructFromString("B X", config1, config2) shouldEqual Some(GameAction(Paper(), Rock()))

    GameAction.constructFromString("C X", config1, config2) shouldEqual Some(GameAction(Scissors(), Rock()))
  }
  ...
  "GameAction.constructFromString" should "result in a success for cases where action and result  are formed" in {
    ...
    GameAction.constructFromString("A X", config1, config2) shouldEqual Some(GameAction(Rock(), Scissors()))

    GameAction.constructFromString("A Y", config1, config2) shouldEqual Some(GameAction(Rock(), Rock()))

    GameAction.constructFromString("A Z", config1, config2) shouldEqual Some(GameAction(Rock(), Paper()))

    GameAction.constructFromString("B X", config1, config2) shouldEqual Some(GameAction(Paper(), Rock()))

    GameAction.constructFromString("C X", config1, config2) shouldEqual Some(GameAction(Scissors(), Paper()))
  }
}
