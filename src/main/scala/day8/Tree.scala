...
case class Tree(col: Int, row: Int, height: Int) {
   ...
   *
   * @param treesInFront tree List in the order they are in front. This the last element of the list is the most
  ...
  def visibleTreesInFront(treesInFront: List[Tree] = List.empty): Int = {
    ...
    def f(trees: List[Tree], result: Int = 0): Int = trees match {
      case Nil => result
      case head :: tail if head.height >= height => result + 1 // hit a tree which is longer, time to stop!
