...
case class Forest(trees: List[Tree]) {
  ...
  /** @param tl
    * @return
    *   true if the tree is visible from any of the edges.
    */
  ...
  def findTreesFromLeft(tl: Tree): List[Tree] =
    trees
      .filter(t => t.col < tl.col && t.row == tl.row)
...
object Forest {
  ...
  /** @param strings
    *   two dimensional string based on the question's input which helps in building the forest
    * @return
    */
