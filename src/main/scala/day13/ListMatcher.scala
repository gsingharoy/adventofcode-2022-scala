...
object ListMatcher {
  ...
  def compareStrLists(leftStr: String, rightStr: String): Boolean = {
    ...
    /** These are mutable variables. The idea is to keep a track of all visited elements and also
      * mark the ones which are evaluated
      */
    ...
    def unEvaluatedItems(
        lStr: String,
        rStr: String,
        currPos: ListElemPosition
    ): (List[String], List[String], List[ListElemPosition]) = {

      @tailrec
      def f(
          l: List[String],
          r: List[String],
          c: List[ListElemPosition]
      ): (List[String], List[String], List[ListElemPosition]) = (l, r, c) match {
        case (l, r, c) if (l.isEmpty || r.isEmpty || c.isEmpty) => (l, r, c)
        case (lh :: lt, rh :: rt, ceh :: cet) =>
          if (evaluatedElements.contains(ceh))
            f(lt, rt, cet)
    ...
    /** Function to mark elements as evaluated. This helps in not going through the same elements
      * again.
      *
      * @param currPos
      */
    def markElementsAsEvaluated(currPos: ListElemPosition): Unit = {
      ...
      currPos.parentPos.foreach(parentPos => {
        val evaluatedChildrenCountBreaths: List[Int] = evaluatedElements
          .filter(_.depth == parentPos.childDepth)
          .distinct
          .map(_.breadth)
          .toList
        val unvisitedChildrenCount: Int = discoveredElements.distinct
          .count(e =>
            e.depth == parentPos.childDepth && !evaluatedChildrenCountBreaths.contains(e.breadth)
          )

        if (
          unvisitedChildrenCount == 0
        ) // there are no unvisited children left. Mark the parent now as visited
    ...
    /** This method analyzes the breadth of a list.
      ...
      * @param rList
      * @return
      *   1 where the items are in order.
      * -1 when the items are not in order. 0 when the items are not enough to be found a result
      */
    def breadthF(lList: List[String], rList: List[String], elements: List[ListElemPosition]): Int =
      (lList, rList, elements) match {
        ...
        case (lh :: lTail, rh :: rTail, cPos :: tailElements) => {
          val result: Option[Int] = (lh, rh) match {
            ...
            case (lStr, rStr) =>
              (lStr.toIntOption, rStr.toIntOption) match {
                case (Some(l), Some(r)) if (l < r) =>
                  Some(1) // the program will stop now as the bytes are in order
                case (Some(l), Some(r)) if (l > r) =>
                  Some(-1) // the program will stop now as the bytes are not in order
                case _ => {
    ...
    /** Analyzes the depth of the list. If it finds sub elements then it calls the breadthF function
      ...
      * @param currPos
      * @return
      *   -1 when the bytes are not in order 0 when no result has been found. 1 when the bytes are
      *   in order
      */
    def depthF(lstr: String, rstr: String, currPos: ListElemPosition): Int =
      unEvaluatedItems(lstr, rstr, currPos) match {
        case (l, r, e) =>
          breadthF(l, r, e) match {
            case 0 => {
              if (!addedEvaluation) {
    ...
    def exec(counter: Int): Boolean = {
      ...
      (
        depthF(leftStr, rightStr, ListElemPosition.Root),
        evaluatedElements.contains(ListElemPosition.Root)
      ) match {
