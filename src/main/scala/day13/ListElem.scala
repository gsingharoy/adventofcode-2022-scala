...
/** Represents a List element with it's current depth and breadth. This case class is useful to
  * count the elements which have been visited.
  *
  * @param elem
  * @param pos
  * @param parentPos
  */
case class ListElem(elem: String, pos: ListElemPosition) {

  def children: List[ListElem] = {
    if (ListElem.isInSingleBracket(elem)) {
      ListElem
        .breakStr(ListElem.removeBrackets(elem))
        .zipWithIndex
        .map({ case (s, i) =>
          ListElem(elem = s, pos = ListElemPosition(depth = this.pos.childDepth, breadth = i))
        })
    } else {
...
object ListElem {
  ...
  /** Breaks the string in such parts that each element is either in a bracket or is a full set in
    * itself
    *
    * @param str
    * @return
  ...
  def breakStr(str: String): List[String] = {
    ...
    @tailrec
    def f(s: List[Char], cache: List[Char], depth: Int, result: List[String]): List[String] =
      s match {
...
case class ListElemPosition(depth: String, breadth: Int) {
  ...
  lazy val parentPos: Option[ListElemPosition] = depth.split("/").toList.reverse match {
    ...
    case head :: tail => head.toIntOption.map(i => ListElemPosition(tail.reverse.mkString("/"), i))
