...
object CommandUtils {
  ...
  /** @param directoryPath
    *   Pass the current path of the directory
    * @param allFiles
  ...
  def constructDirectoryList(strCommands: List[String]): List[OSFile] = {
    ...
    def constructFiles(
        ...
        result: List[OSFile] = List.empty
    ): (List[String], List[OSFile]) = {
      commands match {
        case Nil => (List.empty, result)
        case head :: tail if (isExecutableCommand(head)) =>
          (head :: tail, result) // start of new commands and end of directory structure
        case head :: tail =>
          OSFile.constructFromString(currPath, head) match {
            case Some(file) => {
          ...
          case Some(c) =>
            c match {
              ...
              case cd: ChangeDirectory =>
                executeCommands(
                  changeDirectory(cd, currPath),
                  tail,
                  result
                ) // execute a change in directory
            }
