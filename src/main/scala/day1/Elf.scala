...
object Elf {
  ...
  /**
   *
   * @param calories list of string calories.
   * @return Return an Elf where first the string values are converted to an Int values.
   *         In case it is an invalid string, then that calorific value is ignored
   */
  ...
  def constructElves(strElves: List[String]): List[Elf] = {
    ...
    def attemptToCompleteElf(curr: String, cache: List[String]): Either[Elf, List[String]] = curr match {
      case "" => Left(Elf.fromStrings(cache.reverse))
      case _ => Right(curr :: cache)
    }

    @tailrec
    def f(elves: List[String], cache: List[String] = List.empty, result: List[Elf] = List.empty): List[Elf] = elves match {
      case Nil => Elf.fromStrings(cache) :: result
      case head :: Nil => attemptToCompleteElf(head, cache) match {
        case Left(e) => e :: result
        case Right(s) => Elf.fromStrings(s.reverse) :: result
      }
      case head :: tail => attemptToCompleteElf(head, cache) match {
