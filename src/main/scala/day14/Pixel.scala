...
case class Position(x: Int, y: Int) extends Ordered[Position] {
  ...
  /** Function to return positions of other points from the current point to draw a straight line to
    * them
    *
    * @param endPos
    * @return
...
object Brick {
  ...
  def fromString(str: String): List[Brick] = {
    ...
    @tailrec
    def f(strs: List[String], lastPos: Option[Brick], result: List[Brick]): List[Brick] =
      (strs, lastPos) match {
        case (Nil, _) => result
        case (head :: tail, None) => {
