...
case class YCoordinateRange(y: Int, xStart: Int, xEnd: Int) extends Ordered[YCoordinateRange] {
  ...
  def split(coordinate: Coordinate): List[YCoordinateRange] = (coordinate.x, coordinate.y) match {
    case (x, y) if y != this.y || x < this.xStart || x > this.xEnd =>
      List(this) // the coordinate is not within the range
  ...
  /** Build a list of ranges where they are attempted to be merged together. Returns a None value if
    * no merges are possible
    *
    * @param that
    * @return
    */
  def merge(that: YCoordinateRange): Option[YCoordinateRange] =
    ((this.xStart, this.xEnd), (that.xStart, that.xEnd)) match {
      case ((x1, x2), (x3, x4)) if x1 >= x3 && x2 <= x4 =>
        Some(that) // curr range is completely within the that
      case ((x1, x2), (x3, x4)) if x3 >= x1 && x4 <= x2 =>
        Some(this) // that range is completely within the curr one
      case ((x1, x2), (x3, x4))
          if !Range.inclusive(x3, x4).contains(x1) && Range.inclusive(x3, x4).contains(x2) =>
        Some(this.copy(xEnd = x4))
      case ((x1, x2), (x3, x4))
          if Range.inclusive(x3, x4).contains(x1) && !Range.inclusive(x3, x4).contains(x2) =>
        Some(that.copy(xEnd = x2))
      case ((x1, x2), (x3, x4))
          if !Range.inclusive(x1, x2).contains(x3) && Range.inclusive(x1, x2).contains(x4) =>
  ...
  def applyLimit(xMin: Int, xMax: Int): Option[YCoordinateRange] =
    if (
      !Range
        .inclusive(xMin, xMax)
        .contains(this.xStart) && Range.inclusive(xMin, xMax).contains(this.xEnd)
    )
      ...
      Range
        .inclusive(this.xStart, this.xEnd)
        .contains(xMin) && Range.inclusive(this.xStart, this.xEnd).contains(xMax)
    )
