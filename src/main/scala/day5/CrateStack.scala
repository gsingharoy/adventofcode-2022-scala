...
/** Represents a Crate stack where multiple moves would be made by the crane
  *
  * @param id
  *   Unique integer identifier to identify the CrateStack
  * @param crates
  *   Represents the crates. Here the crates are such that the first element is the one on the top.
  */
case class CrateStack(id: Int, crates: List[Char]) extends Ordered[CrateStack] {
  ...
  /** Function to remove some items from the CrateStack and also return the removed items
    ...
    * @param totalItems
    * @return
    *   (List[Char], CrateStack) which is a tuple of the crates removed in the order they are
    *   supposed to be moved. The 2nd tuple, which is the CrateStack represents the left state of
    *   the Crate
    */
  def removeItems(totalItems: Int, with9001: Boolean = false): (List[Char], CrateStack) = {
    ...
    def f(
        items: List[Char],
        itemsLeft: Int,
        result: List[Char] = List.empty
    ): (List[Char], CrateStack) =
      if (itemsLeft <= 0)
  ...
  /** Add items to the current crate stack. Please note that items are supposed to be added such
    * that the first item goes to the top of the pile
    *
    * @param items
    * @return
    */
