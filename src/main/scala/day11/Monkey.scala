...
/** Represents a monkey
  *
  * @param id
  *   unique identifier of the monkey. The value is 0 to 9
  * @param items
  * @param operation
  * @param test
  */
...
object Monkey extends Ordering[Monkey] {
  ...
  def fromStrings(strings: List[String], smartCorrector: Boolean = false): Option[Monkey] = {
    ...
    def monkeyItems(str: String): List[BigInt] = str match {
      case s"Starting items: ${sItems}" =>
        sItems.split(",").toList.map(_.trim).flatMap(_.toIntOption).map(BigInt(_))
      case _ => List.empty
    }
    ...
    strings.map(_.trim) match {
      ...
      case head :: itemsStr :: opStr :: testStrings =>
        (
          monkeyId(head),
          monkeyItems(itemsStr),
          MonkeyOperation.fromString(opStr),
          MonkeyTest.fromStrings(testStrings)
        ) match {
          case (Some(id), items, Some(op), Some(mt)) if items.nonEmpty =>
            Some(
              Monkey(
                id = id,
                items = items,
...
/** Represents a case class for an operation such that sub op obj. eg, if sub is old, obj is 90 and
  * op is * then "old * 90"
  *
  * @param sub
  * @param op
...
case class MonkeyOperation(sub: OpObject, op: Operator, obj: OpObject) {
  ...
  def calc(old: BigInt): BigInt = op match {
    ...
    case _ =>
      -1 // This is an error case. Ideally it should not exist as op is a typed value and no funny values would be injected
  }
}
  ...
  def fromString(str: String): Option[MonkeyOperation] = str.trim match {
    case s"Operation: new = ${sSub} ${sOp} ${sObj}" =>
      (OpObject.fromString(sSub), Operator.fromString(sOp), OpObject.fromString(sObj)) match {
        case (Some(sub), Some(op), Some(obj)) =>
          Some(MonkeyOperation(sub = sub, op = op, obj = obj))
        case _ => None
      }
    case _ => None
  }
  ...
  def fromStrings(strings: List[String]): Option[MonkeyTest] = strings.map(_.trim) match {
    ...
    case s"Test: divisible by ${sDenom}" :: s"If true: throw to monkey ${sTrueCase}" :: s"If false: throw to monkey ${sFalseCase}" :: _ =>
      (sDenom.toIntOption, sTrueCase.toIntOption, sFalseCase.toIntOption) match {
        case (Some(denom), Some(trueCase), Some(falseCase)) =>
          Some(
