...
object MonkeyUtils {
  ...
  /** Utility function to calculate monkey business. This makes total moves and in the end takes the
    * top two monkeys which have inspected the most items and returns the result
    *
    * @param allMonkeys
    * @param rounds
    * @return
    */
  def calculateMoneeyBusiness(
      allMonkeys: List[Monkey],
      rounds: Int,
      smartCorrector: Boolean = false
  ): Option[BigInt] =
    completeInspections(allMonkeys, rounds, smartCorrector)
    ...
    def fMonkey(ids: List[Int], monkeys: List[Monkey]): List[Monkey] = ids match {
      ...
      case head :: tail =>
        fMonkey(
          ...
          monkeys = monkeys
            .find(_.id == head)
            .map(m => makeMoves(m, monkeys, smartCorrector)._2)
            .getOrElse(monkeys)
        )
  ...
  /** Make moves for a particular monkey. The move continues till all the items of the monkey
    * becomes empty
    *
    * @param monkey
    * @param allMonkeys
  ...
  def makeMoves(
      ...
      allMonkeys: List[Monkey],
      smartCorrector: Boolean = false
  ): (List[ThrowOperation], List[Monkey]) = {
    val (currMonkey, actions) = monkey.inspectItems match {
      case (m, a) if smartCorrector => {
        ...
        (
          m,
          a.map(to =>
            to.copy(worryItem = WorryUtils.applyCorrections(to.worryItem, divisibleDenominators))
          )
        )
      }
      case (m, a) => (m, a)
    ...
    @tailrec
    def reconstruct(input: List[ThrowOperation], monkeys: List[Monkey]): List[Monkey] =
      input match {
        case Nil          => monkeys
